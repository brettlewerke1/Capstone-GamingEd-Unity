<?php
    ...
    $queryString = "SELECT `Course`.`Course_Tag` FROM RTX.Course JOIN RTX.Admin ON `Admin`.`Admin_CourseID` = `Course`.`Course_ID` JOIN `Account` ON `Account`.`Account_ID` = `Admin`.`Admin_AccountID` `WHERE Account_Username='".$username."';";
